...
public class ChatSocket extends JFrame {
    ...
    public ChatSocket() {
        ...
        try {
            Socket c = new Socket("localhost", 1337);
            zumServer = new PrintWriter(c.getOutputStream(), true);
            vomServer = new BufferedReader(new InputStreamReader(c.getInputStream()));
    ...
    private JPanel setupChatView() {
        ...
        inputListener = new KeyboardListenerSocket(/**textArea, **/nick, zumServer);
        new ChatListenerSocket(textArea, vomServer).start();
